...
(function() {
  ...
  Client = (function(_super) {
    ...
    Client.prototype.connect = function(host, port) {
      ...
      this.client.on("ready", this.onClientConnect.bind(this));
      ...
      client = this.client;
      self = this;
      return ['message', 'pmessage', 'unsubscribe', 'end', 'reconnecting', 'subscribe'].forEach(function(evt) {
    ...
    Client.prototype.onClientConnect = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        sub = _ref[_i];
        _results.push(this.client[sub.action].call(this.client, sub.topic));
      }
      return _results;
    };
  ...
  Client.prototype._sub = function(action, unaction, topics) {
    var addTopics, find, sub, topic, _i, _j, _len, _len1, _ref;
    this.logger.info("sub action=" + action + ",unaction=" + unaction + ",topics=" + topics);
    ...
    for (_i = 0, _len = topics.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        sub = _ref[_j];
        if (sub.action === action && sub.topic === topic) {
          find = true;
          break;
        }
      }
    ...
    if (this.status === "connected") {
      return this.client[action].apply(this.client, addTopics);
    }
  };

  Client.prototype._unsub = function(action, topics) {
    var delTopics, i, sub, topic, _i, _j, _len, _len1, _ref;
    this.logger.info("unsub action=" + action + ",topics=" + topics);
    if (topics === null || topics.length === 0) {
      return;
    }
    delTopics = [];
    for (_i = 0, _len = topics.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        sub = _ref[_j];
        if (sub.unaction === action && sub.topic === topic) {
    ...
    if (this.status === "connected") {
      return this.client[action].apply(this.client, delTopics);
    }
  };
  ...
  Client.prototype['subscribe'] = function(topic) {
    ...
    return this._sub('subscribe', 'unsubscribe', args);
  };

  Client.prototype['psubscribe'] = function(topic) {
    var args;
    args = to_array(arguments);
    return this._sub('psubscribe', 'punsubscribe', args);
  };

  Client.prototype['unsubscribe'] = function(topic) {
    var args;
    args = to_array(arguments);
    return this._unsub('unsubscribe', args);
  };

  Client.prototype['punsubscribe'] = function(topic) {
    var args;
    args = to_array(arguments);
    return this._unsub('punsubscribe', args);
  };
  ...
  RedisSentinelClient = (function(_super) {
    ...
    function RedisSentinelClient(options) {
      ...
      this.talkSentinelPingTime = options.talkSentinelPingtime || 5000;
      this.checkClientTime = options.checkClientTime || 30000;
      this.noMasterPartnerSentinelReconnectTime = options.noMasterPartnerSentinelReconnectTime || 10000;
      this.clients = {};
      cOptions = {
        logger: this.logger,
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        client.on("ready", this._checkReady.bind(this));
      }
    ...
    RedisSentinelClient.prototype._checkReady = function() {
      var allReady, c, cs, _i, _j, _len, _len1, _ref;
      if (this.ready) {
        return;
      }
      allReady = true;
    ...
    RedisSentinelClient.prototype._connectSentinel = function(sentinel) {
      var cOptions, host, logger, port, self;
      host = sentinel.host;
      port = sentinel.port;
      self = this;
      logger = this.logger;
      ...
      this.talkSentinel = new RedisSingleClient.createClient(port, host, cOptions);
      this.talkSentinelStatus = "connecting";
      this.talkSentinel.on('ready', function() {
        var fun, pingFun;
      ...
      this.subSentinel = new RedisSingleClient.createClient(port, host, cOptions);
      this.subSentinelStatus = "connecting";
      this.subSentinel.on('ready', function() {
    ...
    RedisSentinelClient.prototype._reconnectClient = function(master, checkOnly, cb) {
      var clients, isReconnectSentinel, logger, self, talkSentinel;
      if (checkOnly == null) {
        checkOnly = false;
      }
      logger = this.logger;
